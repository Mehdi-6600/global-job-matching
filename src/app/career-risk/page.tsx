...
export default function CareerRiskPage() {
  ...
  const stepsAfterRiskHint = useMemo(() => {
    const map: Record<string, string> = {
      fa: "مراحل زیر پس از بررسی ریسک فعال می‌شوند.",
      ar: "الخطوات أدناه تُفعَّل بعد تشغيل تحليل المخاطر.",
      es: "Los pasos siguientes se activan tras el análisis de riesgo.",
      fr: "Les étapes ci-dessous s'activent après l'analyse de risque.",
      hi: "नीचे दिए गए चरण जोखिम विश्लेषण के बाद सक्रिय होते हैं।",
      de: "Die folgenden Schritte werden nach der Risikoanalyse freigeschaltet.",
      en: "The steps below unlock after you run a risk analysis.",
    };
        ...
        >
          ...
          <div className="space-y-3 pt-1">
            <button
              type="submit"
              disabled={loading}
              className="btn-career-risk w-full inline-flex items-center justify-center gap-2 text-sm py-3.5 disabled:opacity-70"
            >
              {loading ? (
                <>
            ...
            <p className="text-[11px] text-center leading-relaxed px-1 career-card-muted">
              {stepsAfterRiskHint}
            </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <button
                ...
                className={
                  analysis
                    ? "btn-career-roadmap w-full inline-flex items-center justify-center gap-2 text-sm py-3.5 disabled:opacity-70"
                    : "btn-career-disabled w-full inline-flex items-center justify-center gap-2 text-sm py-3.5"
                }
              >
                {roadmapLoading ? (
                  <>
              ...
              <button
                ...
                className={
                  analysis
                    ? "btn-career-migration w-full inline-flex items-center justify-center gap-2 text-sm py-3.5 disabled:opacity-70"
                    : "btn-career-disabled w-full inline-flex items-center justify-center gap-2 text-sm py-3.5"
                }
              >
                {migrationLoading ? (
            ...
            {roadmap && (
              <section className="space-y-4 border-t border-white/10 pt-5">
                ...
                {roadmap.weeks.map((w, i) => (
                  <div
                    key={i}
                    className="career-card-surface rounded-xl p-4 space-y-2"
                  >
                    ...
                    <ul className="space-y-1.5">
                      {w.actions.map((task, j) => (
                        <li
                          key={j}
                          className="text-sm career-card-text flex gap-2"
                        >
              ...
              <section className="space-y-4 border-t border-red-500/20 pt-5">
                ...
                <div className="space-y-3">
                  {migration.countries.map((c, i) => (
                    <div
                      key={i}
                      className="career-card-migration rounded-xl p-4 space-y-2"
                    >
