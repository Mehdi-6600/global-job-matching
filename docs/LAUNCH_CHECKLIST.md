...
- [ ] `OWNER_EMAIL`
- [ ] `RESEND_API_KEY` + `RESEND_FROM_EMAIL` (plain email or `Name <email@domain>`)
- [ ] `SYNC_SECRET` (≥ 32)
- [ ] `CRON_SECRET` (≥ 16)
...
- [ ] Node.js **≥ 20.9** in Project Settings (compatible with `package.json` engines)
...
5. [ ] Rate-limited APIs return `code: "RATE_LIMITED"` + optional `Retry-After` when limited
...
9. [ ] Admin: confirm pending tx → plan shows paid + expiry
...
- [ ] Sync endpoints require Bearer secret (not query string)
...
## Explicitly out of scope for “launch-safe” (optional later)

- [ ] On-chain crypto auto-verification
- [ ] Full browser E2E suite
- [ ] `eslint.ignoreDuringBuilds: false` after zero residual lint across entire repo
